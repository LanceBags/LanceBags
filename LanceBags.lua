...
-- ## NEW DEBUG SYSTEM ##
-- Set this to 'true' to see diagnostic messages in chat.
-- Set it to 'false' to turn them off once everything is working.
...
-- ## NEW DEBUG SYSTEM ##
-- This function will print our debug messages to chat if DEBUG_MODE is true.
function addon:Debug(...)
	...
	print("|cff33ff99LanceBags DEBUG:|r", msg)
end
...
local FAMILY_ICONS = {
	[0x0001] = [[Interface\Icons\INV_Misc_Ammo_Arrow_01]], [0x0002] = [[Interface\Icons\INV_Misc_Ammo_Bullet_05]],
	[0x0004] = [[Interface\Icons\INV_Misc_Gem_Amethyst_02]], [0x0008] = [[Interface\Icons\Trade_LeatherWorking]],
	[0x0010] = [[Interface\Icons\INV_Inscription_Tradeskill01]], [0x0020] = [[Interface\Icons\Trade_Herbalism]],
	[0x0040] = [[Interface\Icons\Trade_Engraving]], [0x0080] = [[Interface\Icons\Trade_Engineering]],
	[0x0100] = [[Interface\Icons\INV_Misc_Key_14]], [0x0200] = [[Interface\Icons\INV_Misc_Gem_BloodGem_01]],
	[0x0400] = [[Interface\Icons\Trade_Mining]], [0x100000] = [[Interface\Icons\Trade_Fishing]],
}
...
addon.ITEM_SIZE = 37; addon.ITEM_SPACING = 4; addon.SECTION_SPACING = addon.ITEM_SIZE / 3 + addon.ITEM_SPACING;
addon.BAG_INSET = 8; addon.TOP_PADDING = 32; addon.HEADER_SIZE = 14 + addon.ITEM_SPACING;

local BACKDROP = {
	bgFile = [[Interface\Tooltips\UI-Tooltip-Background]], edgeFile = [[Interface\Tooltips\UI-Tooltip-Border]],
	tile = false, edgeSize = 16, insets = { left = 3, right = 3, top = 3, bottom = 3 },
}
...
local DEFAULT_SETTINGS = {
	profile = {
		enabled = true, viewMode = "category", showSlotCounter = true, bags = { ["*"] = true },
		...
		skin = {		
			background = "Blizzard Tooltip", border = "Blizzard Tooltip", borderWidth = 16, insets = 3,
			BackpackColor = { 0, 0, 0, 1 }, BankColor = { 0, 0, 0.5, 1 },			
		},
	},
	char = { collapsedSections = { ['*'] = false } },
	realm = { characters = {} }
}


--------------------------------------------------------------------------------
-- ## FINALIZED: Data Scanning and Saving ##
--------------------------------------------------------------------------------

function addon:ScanBagsOnly()
    -- print("|cff33ff99LanceBags:|r Scanning bags.")
    local inventory = {}
    local totalItemsFound = 0
    local hasBags = false
    for bagID = BACKPACK_CONTAINER, NUM_BAG_SLOTS do
        local numSlots = GetContainerNumSlots(bagID)
        self:Debug("Bag " .. bagID .. " has " .. numSlots .. " slots")
        if numSlots and numSlots > 0 then
            ...
            for slot = 1, numSlots do
                ...
                if itemID then
                    ...
                    inventory[itemID] = (inventory[itemID] or 0) + (count or 1)
                    totalItemsFound = totalItemsFound + 1
                    self:Debug("Found itemID " .. itemID .. " in bag " .. bagID .. ", slot " .. slot .. ", count " .. (count or 1))
                end
            end
        end
    end
    -- self:Debug("ScanBagsOnly completed, items: " .. totalItemsFound .. ", inventory: " .. table.concat({k .. "=" .. v for k, v in pairs(inventory)}, ", "))
    return inventory
end

function addon:ScanBankOnly()
    self:Debug("ScanBankOnly started.")
    local inventory = {}
    for slot = 1, NUM_BANKGENERIC_SLOTS do
        local itemID = GetContainerItemID(BANK_CONTAINER, slot)
        if itemID then
...
function addon:SaveBankData(event, ...)
	self:Debug("Bank frame closed. Firing SaveBankData.")
	local characterKey = UnitName("player") .. " - " .. GetRealmName()
	if not self.db.realm.characters[characterKey] then self.db.realm.characters[characterKey] = {} end
	self.db.realm.characters[characterKey].bank = self:ScanBankOnly()
	self:Debug("Saved bank data snapshot for", characterKey)
	self:UpdateInteractingWindow(event, ...)
end

function addon:SaveCharacterData()
    self:Debug("SaveCharacterData triggered (bags/money).")
    local characterKey = UnitName("player") .. " - " .. GetRealmName()
    if not self.db.realm.characters then self.db.realm.characters = {} end
    if not self.db.realm.characters[characterKey] then self.db.realm.characters[characterKey] = { bags = {}, bank = {} } end
    local _, class = UnitClass("player")
    self.db.realm.characters[characterKey].class = class
    local money = GetMoney()
    self.db.realm.characters[characterKey].money = money
    self.db.realm.characters[characterKey].lastUpdate = time()
    local bags = self:ScanBagsOnly()
    local hasBags = next(bags) and true or false
    -- print("|cff33ff99LanceBags:|r SaveCharacterData - Bags: " .. (hasBags and "not empty" or "empty") .. ", Money: " .. money)
    self.db.realm.characters[characterKey].bags = bags
    self:Debug("Saved bags and money for", characterKey)
end

function addon:OnPlayerLogin()
    self:UnregisterEvent("PLAYER_LOGIN")
    print("|cff33ff99LanceBags:|r Scheduling initial character data scan.")
    LibCompat.After(1, function() self:SaveCharacterData() end)
end
...
function addon:OnInitialize()
	...
	-- Persistant handlers
	self.RegisterBucketMessage(addonName, 'LanceBags_ConfigChanged', 0.2, function(...) addon:ConfigChanged(...) end)
	
	-- ## FIX ##
	-- This line is restored. It is the correct, primary trigger to enable the
	-- addon when the player enters the world.
	...
	if PV then
		...
		self.RegisterMessage(PV, "LanceBags_UpdateButton", function(_, button)
...
-- ## FINALIZED: This is the definitive OnEnable function. ##
function addon:OnEnable()
    self.globalLock = false
    self:RegisterEvent('BAG_UPDATE')
    self:RegisterBucketEvent('PLAYERBANKSLOTS_CHANGED', 0, 'BankUpdated')
    self:RegisterMessage('LanceBags_BagOpened', 'LayoutBags')
    self:RegisterMessage('LanceBags_BagClosed', 'LayoutBags')
    ...
    self:RegisterEvent("PLAYER_LOGOUT", function()
        local characterKey = UnitName("player") .. " - " .. GetRealmName()
        local data = self.db.realm.characters[characterKey] or {}
        local hasBags = data.bags and next(data.bags) and true or false
        local money = data.money or 0
        -- print("|cff33ff99LanceBags:|r Logout - Bags: " .. (hasBags and "not empty" or "empty") .. ", Money: " .. money)
        LibCompat.After(1, function() -- Delay to ensure print is visible
            self:SaveCharacterData()
            self:Disable()
        end)
    end)
    self:RegisterEvent("BANKFRAME_CLOSED", "SaveBankData")
    ...
    self:RegisterBucketEvent("BAG_UPDATE", 0.2, "SaveCharacterData")
    ...
    addon:SetSortingOrder(self.db.profile.categorySortingOrder)
    self:Debug('Enabled')
end
...
-- (The rest of the file from here is identical to your original upload, including Reconfigure, OnProfileChanged, UpgradeProfile, and all other functions)

function addon:Reconfigure()
	self:Debug("Profile changed, applying updates.")
	
	-- ## FIX ##
	-- We no longer do a full Disable/Enable cycle here.
	-- Instead, we just call the functions needed to update the addon's state.
	
	self:UpdateFilters()
	self:LayoutBags()
	
	-- We also need to re-apply font settings and skin changes
	self.bagFont:ApplySettings()
	self.sectionFont:ApplySettings()
	for _, bag in self:IterateBags() do
		if bag:HasFrame() then
			bag:GetFrame():UpdateSkin()
		end
	end
end

function addon:OnProfileChanged()
	self:UpgradeProfile(); return self:Reconfigure()
end

function addon:UpgradeProfile()
	local profile = self.db.profile; if profile.laxOrdering == true then profile.laxOrdering = 1 end
	local oldData = profile.anchor
	if oldData then
		local scale = oldData.scale or 0.8; profile.scale = scale
		local newData = profile.positions.anchor
		newData.point = oldData.pointFrom or "BOTTOMRIGHT"; newData.xOffset = (oldData.xOffset or -32) / scale; newData.yOffset = (oldData.yOffset or 200) / scale
		profile.anchor = nil
	end
	if profile.virtualStacks.notWhenTrading == true then profile.virtualStacks.notWhenTrading = 3 end
	if type(profile.rowWidth) == "number" then local rowWidth = profile.rowWidth; profile.rowWidth = { Bank = rowWidth, Backpack = rowWidth } end
	if type(profile.backgroundColors) == "table" then
		profile.skin.BackpackColor = profile.backgroundColors.Backpack; profile.skin.BankColor = profile.backgroundColors.Bank; profile.backgroundColors = nil
	end
	if type(profile.skin) == "table" then
		local skin = profile.skin
		if type(skin.font) == "string" then profile.bagFont.name = skin.font; profile.sectionFont.name = skin.font; skin.font = nil end
		if skin.fontSize then profile.bagFont.size = skin.fontSize; profile.sectionFont.size = skin.fontSize - 4; skin.fontSize = nil end
		if skin.fontBagColor then local bagFont = profile.bagFont; bagFont.r, bagFont.g, bagFont.r = unpack(skin.fontBagColor); skin.fontBagColor = nil end
		if skin.fontSectionColor then local sectionFont = profile.sectionFont; sectionFont.r, sectionFont.g, sectionFont.b = unpack(skin.fontSectionColor); skin.fontSectionColor = nil end
	end
end

function addon:BAG_UPDATE(event, bag) self:SendMessage('LanceBags_BagUpdated', bag) end

function addon:BankUpdated(slots)
	for slot in pairs(slots) do
		if slot > 0 and slot <= NUM_BANKGENERIC_SLOTS then self:SendMessage('LanceBags_BagUpdated', BANK_CONTAINER); return end
	end
end

function addon:CloseSpecialWindows() local bagWasOpen = self:CloseAllBags(); return self.hooks.CloseSpecialWindows() or bagWasOpen end

function addon:GetContainerSkin(containerName)
	local skin = self.db.profile.skin; local r, g, b, a = unpack(skin[containerName..'Color'], 1, 4)
	BACKDROP.bgFile = LSM:Fetch(LSM.MediaType.BACKGROUND, skin.background); BACKDROP.edgeFile = LSM:Fetch(LSM.MediaType.BORDER, skin.border)
	BACKDROP.edgeSize = skin.borderWidth; BACKDROP.insets.left = skin.insets; BACKDROP.insets.right = skin.insets; BACKDROP.insets.top = skin.insets; BACKDROP.insets.bottom = skin.insets
	return BACKDROP, r, g, b, a
end

function addon:ConfigChanged(vars)
	if vars.enabled then if self.db.profile.enabled then self:Enable() else self:Disable() end; return
	elseif not self:IsEnabled() then return
	elseif vars.filter then return self:SendMessage('LanceBags_FiltersChanged')
	else
		for name in pairs(vars) do
			if strmatch(name, 'virtualStacks') then return self:SendMessage('LanceBags_FiltersChanged')
			elseif strmatch(name, 'bags%.') then
				local _, bagName = strsplit('.', name); local bag = self:GetModule(bagName)
				local enabled = self.db.profile.bags[bagName]
				if enabled and not bag:IsEnabled() then bag:Enable() elseif not enabled and bag:IsEnabled() then bag:Disable() end
			elseif strmatch(name, 'rowWidth') then return self:SendMessage('LanceBags_LayoutChanged')
			elseif strmatch(name, '^skin%.font') then return self:UpdateFonts() end
		end
	end
	if vars.categorySortingOrder then addon:SetSortingOrder(self.db.profile.categorySortingOrder)
	elseif vars.oneBagSortingOrder then self:SendMessage('LanceBags_LayoutChanged')
	elseif vars.maxHeight or vars.laxOrdering then return self:SendMessage('LanceBags_LayoutChanged')
	elseif vars.scale then return self:LayoutBags()
	elseif vars.positionMode then return self:UpdatePositionMode()
	else self:SendMessage('LanceBags_UpdateAllButtons') end
end

function addon:SetGlobalLock(locked)
	locked = not not locked
	if locked ~= self.globalLock then
		self.globalLock = locked; self:SendMessage('LanceBags_GlobalLockChanged', locked)
		if not locked then self:SendMessage('LanceBags_LayoutChanged') end
		return true
	end
end
	...
	function addon:UpdateInteractingWindow(event, ...)
		...
		if new ~= current then
			...
			if self.db.profile.virtualStacks.notWhenTrading then
				self:SendMessage('LanceBags_FiltersChanged', 0)
			end
			self:SendMessage('LanceBags_InteractingWindowChanged', new, old)
			self:SendMessage('LanceBags_TimeToCheckAnchorMode')
		end
	end
...
function bagProto:Open()
	...
	if not frame:IsShown() then
		self:Debug('Open')
		frame:Show()
		addon:SendMessage('LanceBags_BagOpened', self.bagName, self)
		return true
	end
end
...
function bagProto:Close()
	if self.frame and self.frame:IsShown() then
		...
		self.frame:Hide()
		CloseMenus()
		addon:SendMessage('LanceBags_BagClosed', self.bagName, self)
...
function bagProto:GetFrame()
	if not self.frame then
		...
		addon:SendMessage('LanceBags_BagFrameCreated', self)
	end
	return self.frame
end
...
function addon:HookBagFrameCreation(target, callback)
	...
	if listen then
		target:RegisterMessage("LanceBags_BagFrameCreated", hook)
	end
end
...
do
	...
	function backpack:LanceBags_InteractingWindowChanged(event, window)
		if window then
...
do
	...
	function bank:LanceBags_InteractingWindowChanged(event, new, old)
	...
	-- ## MODIFIED: The PostClose function has been removed to prevent a double-call error. ##
	-- The BANKFRAME_CLOSED event is now the single source of truth for bank closing actions.
...
function addon:CreateBagAnchor()
	local anchor = self:CreateAnchorWidget(UIParent, "anchor", L["LanceBags Anchor"])
	...
	text:SetText(L["LanceBags Anchor"])
...
function addon:LayoutBags()
	...
	if self.db.profile.positionMode == 'anchored' then
		...
		self:SendMessage('LanceBags_AnchoredLayout')
	else
		ManualBagLayout(self)
		-- print("manual layout")
		self:SendMessage('LanceBags_ManualLayout')
	end
end
...
function addon:ToggleCurrentLayout()
	...
	if self.db.profile.positionMode == 'anchored' then
		...
		-- First call with a duration of 0.5 seconds to remove delay of LanceBags message.
...
function addon:CycleViewMode()
	local currentMode = self.db.profile.viewMode

	if currentMode == "category" then
		self.db.profile.viewMode = "bag"
	elseif currentMode == "bag" then
		self.db.profile.viewMode = "onebag"
	else -- it was 'onebag' or something else
		self.db.profile.viewMode = "category"
	end

	-- Broadcast messages to tell the UI to update
	self:SendMessage('LanceBags_ViewModeChanged')
	self:SendMessage('LanceBags_FiltersChanged', true) -- 'true' forces a full redraw
end
