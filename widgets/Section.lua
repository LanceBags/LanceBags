...
function CompareButtons(a, b)
	...
	if viewMode == 'bag' then
		-- Bag View: Sort by physical slot location
		if not a or not a.slotId then return false end
		if not b or not a.slotId then return true end
		return a.slotId < b.slotId

	elseif viewMode == 'onebag' then
		-- One Bag View: Empty slots first, then use configurable sort method
		local idA, idB = a:GetItemId(), b:GetItemId()

		-- Step 1: Handle empty slots (no item ID)
		local aIsEmpty = not idA
		local bIsEmpty = not idB

		if aIsEmpty and not bIsEmpty then return true end  -- Empty slots come first
		if not aIsEmpty and bIsEmpty then return false end -- Items come after empty slots
		if aIsEmpty and bIsEmpty then return false end   -- Order of empty slots doesn't matter

		-- Step 2: Both are items, get info and apply selected sort order
		local sortOrder = addon.db.profile.oneBagSortingOrder or "quality"
		local nameA, _, qualityA, levelA = GetItemInfo(idA)
		local nameB, _, qualityB, levelB = GetItemInfo(idB)

		if sortOrder == "quality" then
			-- Sort by Quality -> Item Level -> Name
		...
		elseif sortOrder == "level" then
			-- Sort by Item Level -> Quality -> Name
			if levelA ~= levelB then return levelA > levelB end
			if qualityA ~= qualityB then return qualityA > qualityB end
			return nameA < nameB
		...
		-- Category View (Original Logic using the other DB setting)
		local idA, idB = a:GetItemId(), b:GetItemId()
		if idA and idB then
			if idA ~= idB then
