LanceBags display the content bag in one view, gather into several sections using smart filters.
...
LanceBags has been built around, and optimized for, the idea of the "all-in-one" view. Doing anything would require to redesign big parts of the addons, especially the layout code that is quite complex at the first place.

h3. Anything that requires to scan item tooltips

Tooltip scanning is quite CPU-consuming and above all requires asynchronous programming, since item tooltips would not always be available at the time LanceBags should use them. This would raise the complexity of bag filtering be an order of magnitude and turn LanceBags into a CPU hog.
...
LanceBags presents you a nice view of your bags but actually they are in a complete mess. With LanceBags, there is no need and no way to tidy up the items in the actual bags. Guilds usually try to keep their bank tidy. What would happen if some people in a guild used LanceBags for the guild bank ? Well, they would mess up the guild bank content and people NOT using LanceBags would have to deal with that mess. I can hear the screams from there. So that is why I will not implement guild bank.

h3. Alt bags and bank

LanceBags is not an alt bags/bank viewer. If you want to keep track of what your alt owns, I suggest using an addon like "BagSync":http://www.wowinterface.com/downloads/info15351-BagSync.html.
...
I have written two separate modules for LanceBags:
* "LanceBags_Outfitter":http://wow.curse.com/downloads/wow-addons/details/adibags_outfitter.aspx: add filters for "Outfitter":http://wow.curse.com/downloads/wow-addons/details/outfitter.aspx item sets.
* "LanceBags_PT3Filter":http://wow.curse.com/downloads/wow-addons/details/adibags-pt3filter.aspx: add filters based on "LibPeriodicTable-3.1":http://wow.curse.com/downloads/wow-addons/details/libperiodictable-3-1.aspx categories.
